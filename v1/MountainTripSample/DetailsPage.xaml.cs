...
namespace MountainTripTimeSample.Views
{
    public partial class DetailsPage : ContentPage
    {
        ...
        public DetailsPage()
        {
            ...
            for (int i = 1; i < mountains.Count; i++)
            {
                StackLayout stackHeader = new StackLayout()
                {
                    ...
                    Padding       = new Thickness(80,30,1,0) 
                };
        ...
        private async void Traductor_Clicked(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Atraducir.Text))
            {
                if (Atraducir.Text.ToLower() == "hola")
                {
                    Resultado.Text = "Hi";
                }
               

                if (Atraducir.Text.ToLower() == "necesito ayuda")
                {
                    Resultado.Text = "I need help";
                }
                
            }
            else
            {
                await DisplayAlert("Advertencia","Debe ingresar palabra a traducir","Aceptar");
            }
